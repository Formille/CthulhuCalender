...
class ActionType(str, Enum):
    """행동 유형 (주사위 기호)"""
    COMBAT = "전투 (권총)"  # Pistol
    INVESTIGATION = "조사 (돋보기)"  # Lupe
    SEARCH = "수색 (손전등)"  # Flashlight
...
{f"          * 일요일 조우 성공률: {sunday_success_percent}% - 일요일 조우의 성공률이 높다면(70% 이상) 존 밀러는 자신감을 가지고 위협에 맞서고 있으며, 스토리는 더 적극적이고 공격적인 방향으로 전진해야 합니다. 성공률이 낮다면(50% 미만) 존 밀러는 좌절감과 절망감에 빠져있으며, 스토리는 더 어둡고 절망적인 방향으로 전진해야 합니다." if sunday_total_count > 0 else ""}
          * 전체 조우 성공률: {overall_success_percent}% - 전체 조우 성공률이 높다면(70% 이상) 조사가 순조롭게 진행되고 있으며, 단서들이 잘 연결되고 있습니다. 성공률이 낮다면(50% 미만) 조사가 막히고 있으며, 위협이 점점 더 커지고 있습니다.
          * 광기 상태: {madness_state_text} - 광기 상태에 따라 스토리의 톤과 방향이 달라져야 합니다. 광기가 심각할수록 현실과 환상의 경계가 무너지고, 더 어둡고 절망적인 내용이 포함되어야 합니다.
        ...
        - 텍스트 강조: 중요한 단어나 구절은 마크다운 형식으로 강조하세요.
          * 굵게: **텍스트** 또는 __텍스트__ 형식 사용
          * 이탤릭: *텍스트* 또는 _텍스트_ 형식 사용
